...
## Dialogue player-body preservation

The former recovery gate explicitly excluded dialogue and required gameplay mode, so dialogue's first-person switch could hide the third-person body despite cameraBlend remaining 1. Recovery now includes dialogue menus, including seated conversations, while Pip-Boy states remain excluded by cameraActive. Added third_person_body telemetry and regression checks for dialogue, seated dialogue, unrelated menus, existing third person and released camera ownership.

Live diagnostic process 15028 reached interior dialogue with dialogue=true, game_mode=false and camera_blend=1. Main-loop recovery reported repeated third-person restores, while a separate read observed native byte 64C returning to zero. This shows that recovery alone fights a recurring native POV request. A final additional guard wraps 25 statically identified direct calls to 950110; each site's call target must match before patching. Only player requests during owned isometric dialogue are changed to third person. Original routine and return are retained; Pip-Boy, disabled mode and non-dialogue callers pass through. No blanket scene visibility flags or actor model reloads are applied.

Final Release build and combat regression tests pass. The final call-site guard DLL is staged as runtime/MojaveIsoNative.pending.dll for the next launcher start, because process 15028 remains running with the earlier recovery-only build. Final guard installation and visual dialogue stability are not yet live verified. Do not treat the older runtime screenshot as evidence; the capture file did not refresh during the dialogue menu.

## Damage notification timing and stable close melee facing

User live test on diagnostic process 35892 registered nine player damage callbacks but zero queued popups. Health at callback entry and exit was identical: successive entries 57.65, 50.30, 42.95 ... -1.149994, with signed amount -7.349999. Thus this notification arrives after health has been applied; the previous before/after wrapper comparison could never detect these hits. Popup amount now derives from the native negative delta and health at notification entry, reconstructing pre-hit health to cap the killing blow. Positive/zero/invalid deltas are ignored. The original native notification still runs with unchanged arguments. Popup tile creation failures and total callback/player-hit/queued-popup counts are exposed for diagnosis.

Melee no longer faces from the animated muzzle/head position. It faces from player feet toward target feet (actor targets), holds its prior yaw inside a 12-unit overlap dead zone, uses the shortest wrapped yaw difference and limits rotation to 360 degrees/sec with a capped timestep. Ranged muzzle-based aiming is unchanged. Applies to Alt preview, ordinary attack updates and retained single-shot action windows. Regression checks cover overlap, rate limit and crossing +/-pi.

Release build and combat/damage tests passed. Tests include the observed 7.35-damage notification and 6.2-health killing blow. DLL and damage.xml installed while FalloutNV was closed. The live test established the notification timing bug; final popup visibility and close machete behavior still need a visual gameplay check after launch. Latest build also includes the previously staged dialogue body guards.

## Popup positioning, Alt cover picking and melee submission

User confirmed damage numbers now display; that working state was committed as 2d01793 before these follow-up edits. Centred native TileText was positioned as though x were its centre, but x is the start of its 160-unit wrap box. Popup x now subtracts half that width. Popups retain an actor form ID (not an actor pointer), resolve its current same-cell head position plus clearance and rise above it; missing actors retain their recorded fallback position. This keeps short-lived labels with moving victims.
