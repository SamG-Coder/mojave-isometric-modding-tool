...
inline bool plan(engine::Vec start,engine::Vec goal,float tolerance,std::vector<engine::Vec>& path,size_t& count,unsigned& expanded){
 ...
 std::vector<navigation::MeshTriangle> triangles;
 for(size_t m=0;m<meshes.size()&&visited.size()<128;m++){
  ...
  auto extras=at<char*>(mesh,0x4C);auto ne=at<uint32_t>(mesh,0x50);
  for(unsigned i=0;i<nt;i++){auto t=ts[i];if(t.flags&0x20)continue;bool valid=true;navigation::MeshTriangle copy{};copy.mesh=id;copy.index=i;
   ...
   // Triangle flag bits 0..2 select external connection indices, not local
   // triangle indices. Resolve the exact destination triangle from that record.
   for(int j=0;j<3;j++)if(t.flags&(1u<<j)){
    copy.side[j]=-1;auto edgeIndex=uint16_t(t.side[j]);
    if(extras&&ne<65536&&edgeIndex<ne){auto edgeInfo=at<void*>(extras,edgeIndex*12+4);
     if(edgeInfo&&!(at<uint32_t>(edgeInfo,8)&0x10))copy.external[j]={at<uint32_t>(edgeInfo,0),int(uint16_t(at<int16_t>(extras,edgeIndex*12+8)))};
    }
   }
   if(valid)triangles.push_back(copy);
  }
  if(extras&&ne<65536)for(unsigned e=0;e<ne;e++){
   auto nextInfo=at<void*>(extras,e*12+4);if(!nextInfo||(at<uint32_t>(nextInfo,8)&0x10))continue;
   auto nextId=at<uint32_t>(nextInfo,0);auto next=reference(nextId);if(next&&at<uint8_t>(next,4)==0x43){if(!visited.count(next)&&meshes.size()<4096)meshes.push_back(next);}
  }
 }
 navigation::MeshGraph graph;graph.build(triangles);count=graph.size();bool found=graph.find(start,goal,tolerance,path);expanded=graph.expanded;return found;
}
}
