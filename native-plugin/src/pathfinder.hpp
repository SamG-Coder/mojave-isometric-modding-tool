...
class Search {
    ...
    std::map<PointKey,FloorSample> floors;
    std::map<std::pair<PointKey,PointKey>,bool> edges;
    std::map<std::tuple<int,int,int>,int> ids;std::vector<Node> nodes;std::priority_queue<Entry> open;
...
public:
    ...
    void cancel(){state=State::Idle;testingDirect=false;directRoute=false;directPath.clear();path.clear();open={};nodes.clear();ids.clear();floors.clear();edges.clear();}
    ...
    void step(const Ground& ground,const Clear& clear,unsigned budget=6,float milliseconds=2.f){
