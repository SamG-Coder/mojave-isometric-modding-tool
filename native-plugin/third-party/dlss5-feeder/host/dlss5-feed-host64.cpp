...
static bool Evaluate(ID3D12Resource *color, ID3D12Resource *output, ID3D12Resource *depth, ID3D12Resource *mv,
                     UINT w, UINT h_, int reset, float mvsx, float mvsy, float jitter_x = 0.0f, float jitter_y = 0.0f, ID3D12Resource *historyMask = nullptr)
{
    ...
    ep.pInBiasCurrentColorMask = historyMask;
    static bool maskLogged=false;if(historyMask&&!maskLogged){Log("[host] NGX history rejection mask connected (R8, full resolution)");maskLogged=true;}
        ...
        if (tag == 'B')
        {
            ...
            if (host_creates_b)
            {
                ...
                const DXGI_FORMAT fmt[FEED_SLOTS] = {
                    ...
                    DXGI_FORMAT_R32_FLOAT, DXGI_FORMAT_R16G16_FLOAT, DXGI_FORMAT_R8_UNORM };
            ...
            else
            {
                ...
                for (int i = 0; i < FEED_SLOTS && ok; ++i)
                {
                    ...
                    const DXGI_FORMAT want_fmt =
                        ...
                        i == FEED_DEPTH  ? DXGI_FORMAT_R32_FLOAT : i == FEED_MASK ? DXGI_FORMAT_R8_UNORM : DXGI_FORMAT_R16G16_FLOAT;
            ...
            if (transport_only)
            {
                if (BeginCommands())
                {
                    ...
                    EndCommands();
                                ...
                                h.width, h.height, fm.reset ? 1 : 0, mvsx, mvsy, fm.jitter_x, fm.jitter_y, h.tex[FEED_MASK]);
